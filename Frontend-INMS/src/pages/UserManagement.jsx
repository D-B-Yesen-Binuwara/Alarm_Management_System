...
const initialUsers = [
  { id: 1, username: 'jdoe', fullName: 'Jane Doe', serviceNumber: '123456', role: 'Network Engineer', region: 'Western', province: 'Colombo', lea: 'City' },
  { id: 2, username: 'mthompson', fullName: 'Mike Thompson', serviceNumber: '123457', role: 'Support', region: 'Central', province: 'Kandy', lea: 'Town' },
  { id: 3, username: 'snguyen', fullName: 'Sarah Nguyen', serviceNumber: '123458', role: 'Admin', region: 'Southern', province: 'Galle', lea: 'Coast' },
  { id: 4, username: 'rpatel', fullName: 'Raj Patel', serviceNumber: '123459', role: 'Developer', region: 'Western', province: 'Gampaha', lea: 'North' },
  { id: 5, username: 'lchen', fullName: 'Lisa Chen', serviceNumber: '123460', role: 'Analyst', region: 'Eastern', province: 'Trincomalee', lea: 'Port' },
  { id: 6, username: 'pndusr', fullName: 'John Pending', serviceNumber: '123461', role: 'Developer', status: 'Pending', region: 'Western', province: 'Gampaha', lea: 'South' },
  { id: 7, username: 'bjohnson', fullName: 'Bob Johnson', serviceNumber: '123462', role: 'Network Engineer', status: 'Pending', region: 'Central', province: 'Matale', lea: 'East' },
  { id: 8, username: 'klee', fullName: 'Karen Lee', serviceNumber: '123463', role: 'Support', status: 'Pending', region: 'Eastern', province: 'Ampara', lea: 'West' },
  { id: 9, username: 'dwilliams', fullName: 'David Williams', serviceNumber: '123464', role: 'Analyst', status: 'Pending', region: 'Southern', province: 'Matara', lea: 'North' },
]

const emptyForm = { username: '', fullName: '', serviceNumber: '', role: '', region: '', province: '', lea: '' }
...
export default function UserManagement() {
  const [users, setUsers] = useState(() => {
    try { 
      const s = localStorage.getItem('inms-users'); 
      const parsed = s ? JSON.parse(s) : [];
      const merged = parsed.map(u => {
        const initU = initialUsers.find(i => i.username === u.username);
        return initU ? { ...initU, ...u, serviceNumber: u.serviceNumber || initU.serviceNumber } : u;
      });
      initialUsers.forEach(iu => {
        if (!merged.find(u => u.username === iu.username)) merged.push(iu);
      });
      return merged.length > 0 ? merged : initialUsers;
  ...
  const filteredUsers = useMemo(() => users.filter(u => {
    ...
    return (u.fullName?.toLowerCase().includes(s) || u.username?.toLowerCase().includes(s) || u.serviceNumber?.includes(s))
  ...
  const showRegion = form.role === 'Region Officer' || form.role === 'Province Officer' || form.role === 'LEA Officer' || ['Network Engineer', 'Developer', 'Support', 'Analyst'].includes(form.role);
  const showProvince = form.role === 'Province Officer' || form.role === 'LEA Officer' || ['Network Engineer', 'Developer', 'Support', 'Analyst'].includes(form.role);
  const showLEA = form.role === 'LEA Officer' || ['Network Engineer', 'Developer', 'Support', 'Analyst'].includes(form.role);

  const isValid = useMemo(() => {
    return !!(
      form.username?.trim() &&
      form.fullName?.trim() &&
      form.serviceNumber?.trim().length === 6 &&
      form.role?.trim() &&
      (!showRegion || form.region?.trim()) &&
      (!showProvince || form.province?.trim()) &&
      (!showLEA || form.lea?.trim())
    );
  ...
  const openModal = (mode, user = null) => {
    setModalMode(mode)
    if (mode === 'edit' && user) { setForm({ username: user.username, fullName: user.fullName, serviceNumber: user.serviceNumber, role: user.role, region: user.region, province: user.province, lea: user.lea }); setEditingUser(user) }
  ...
  const handleSubmit = () => {
    if (!isValid) { setMessage('Please complete all fields (Service Number must be 6 digits).'); return }
    if (modalMode === 'add') {
      const nextId = users.length ? Math.max(...users.map(u => u.id)) + 1 : 1
      setUsers(p => [...p, { id: nextId, ...form }])
  ...
  const handleApprove = (id) => {
    ...
    setMessage('Request approved!'); setTimeout(() => setMessage(''), 3000)
  }

  const handleReject = (id) => {
    setUsers(p => p.map(u => u.id === id ? { ...u, status: 'Rejected' } : u))
    setMessage('Request rejected!'); setTimeout(() => setMessage(''), 3000)
  }

  const statCards = [
    { label: 'Current Users', value: stats.current, icon: <><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></> },
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-slate-500">{user.serviceNumber}</div>
                    </div>
                  </div>
                </td>
                <td className="px-6 py-4">
                  ...
                  <div className="flex gap-2">
                    {viewMode === 'pending' ? (
                      <>
                        ...
                        {user.status === 'Rejected' && (
                          <button onClick={() => handleDelete(user.id)} title="Delete permanently"
                            className="p-1.5 rounded-md text-red-400 hover:bg-red-50 hover:text-red-600 transition">
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={closeModal}>
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-6 py-5 flex flex-col gap-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-medium text-slate-700">Service Number (6 digits) *</label>
                  <input type="text" maxLength={6} value={form.serviceNumber} onChange={e => handleInputChange('serviceNumber', e.target.value.replace(/\\D/g, ''))} placeholder="e.g. 123456" className={inputCls} />
                </div>
                <div className="flex flex-col gap-1.5">
              ...
              <div className="grid grid-cols-3 gap-4">
                {showRegion && (
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input type="text" value={form.region} onChange={e => handleInputChange('region', e.target.value)} placeholder="Region" className={inputCls} />
                  </div>
                )}
                {showProvince && (
                  <div className="flex flex-col gap-1.5">
                    <label className="text-sm font-medium text-slate-700">Province *</label>
                    <input type="text" value={form.province} onChange={e => handleInputChange('province', e.target.value)} placeholder="Province" className={inputCls} />
                  </div>
                )}
                {showLEA && (
                  <div className="flex flex-col gap-1.5">
                    <label className="text-sm font-medium text-slate-700">LEA *</label>
                    <input type="text" value={form.lea} onChange={e => handleInputChange('lea', e.target.value)} placeholder="LEA" className={inputCls} />
                  </div>
                )}
              </div>
            </div>
